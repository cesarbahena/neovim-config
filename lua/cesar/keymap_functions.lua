...
function Keymaps.remap(table_of_remaps)
	for mode, remaps in pairs(table_of_remaps) do
		for _, remap in ipairs(remaps) do
			...
			if lhs then
				vim.keymap.set(mode, lhs, remap[3], { desc = remap[1] })
			else
				vim.keymap.set(mode, remap[3], remap[3], { desc = remap[1] })
			end
		end
	end
end

function Keymaps.ymap_telescope(key, fn, opts, buffer)
	local id = vim.api.nvim_replace_termcodes(key .. fn, true, true, true)
	Telescopes = Telescopes or {}
	Telescopes[id] = opts or {}

	local rhs = string.format("<cmd>lua Plugin 'telescope.finders'['%s'](Telescopes['%s'])<CR>", fn, id)
...
function Keymaps.map_numpad(params)
	...
	for i = params["i"], params["n"] do
		for j = params["j"], params["m"] do
			vim.keymap.set("", params["prefix"] .. tens[i + 1] .. params["digits"][j + 1], non_zero(i) .. j)
		end
	end
end

Plugin("telescope.pickers")
